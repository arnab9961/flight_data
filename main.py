...
from app.models.flight_search import (
    FlightSearch,
    MultiCityRequest,
    OfferDetails
)
from app.models.time_specs import TimeSpec
from app.api.client import DuffelClient
...
app = FastAPI(
    ...
    description="API for searching flights using Duffel API",
    version="1.0.0",
)
...
async def search_flights(params: FlightSearch) -> JSONResponse:
    ...
    try:
        ...
        # Get all offers (limit to 50 to manage response size)
        for offer in response.get('offers', [])[:50]:
            offer_details = {
...
async def get_offer_details(offer_id: str) -> JSONResponse:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error getting offer details: {str(e)}")

@app.post("/search-multi-city")
async def search_multi_city(params: MultiCityRequest) -> JSONResponse:
    ...
    try:
        ...
        # Use async context manager with longer timeout
        async with flight_client as client:
            response = await client.create_offer_request(
                slices=slices,
                cabin_class=params.cabin_class,
...
@app.get("/")
def root():
    return {"status": "ok", "message": "Flight Search API is running"}