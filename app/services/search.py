                ...
                for slice in offer.get('slices', []):
                    ...
                    if segments:
                        slice_details = {
                            ...
                            'stops_description': 'Non-stop' if len(segments) == 1 else f'{len(segments) - 1} stop{"s" if len(segments) - 1 > 1 else ""}',
                            'connections': [],
                            'flights': []  # New field to hold flight numbers
                        }
                        
                        # Extract flight names/numbers for each segment
                        for segment in segments:
                            flight_info = {
                                'flight_number': segment.get('marketing_carrier_flight_number', ''),
                                'carrier_code': segment.get('marketing_carrier', {}).get('iata_code', ''),
                                'full_flight_code': f"{segment.get('marketing_carrier', {}).get('iata_code', '')}{segment.get('marketing_carrier_flight_number', '')}",
                                'aircraft': segment.get('aircraft', {}).get('name', 'Unknown Aircraft'),
                                'segment_departure': segment.get('departing_at'),
                                'segment_arrival': segment.get('arriving_at'),
                                'origin_terminal': segment.get('origin_terminal'),
                                'destination_terminal': segment.get('destination_terminal')
                            }
                            slice_details['flights'].append(flight_info)
